...
export function BlogPostsListTable() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(adminApiUrl(`/api/v1/admin/blog?${params}`));
    const json = await res.json();
    setLoading(false);

    if (!res.ok) {
      toast.error(json.error?.message ?? "Failed to load posts");
      return;
    }
  ...
  async function deleteOne(id: string, title: string) {
    ...
    setDeleting(true);
    const res = await fetch(adminApiUrl(`/api/v1/admin/blog/${id}`), { method: "DELETE" });
    setDeleting(false);
    if (!res.ok) {
      const json = await res.json();
  ...
  async function bulkDelete() {
    ...
    setDeleting(true);
    const res = await fetch(adminApiUrl("/api/v1/admin/blog/bulk-delete"), {
      method: "POST",
    ...
    toast.success(`Deleted ${json.data?.deleted ?? selected.size} post(s)`);
    router.refresh();
    await load();
  }
