...
} from "@/features/distributors/schemas/distributors-content.schema";
import {
...
export async function GET() {
  try {
    ...
    return jsonOk(await getAdminDistributorsEditorData());
  } catch (e) {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (content) {
      await saveDistributorsContent(content as DistributorsPageContent);
    }
    ...
    if (page) {
      ...
      await prisma.page.upsert({
        ...
        update: {
          title: String(page.title ?? "International Distributors"),
    ...
    revalidatePath("/international-distributors");
    revalidatePath("/international-distributors/");
